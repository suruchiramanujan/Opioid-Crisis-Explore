...
These first ___ are the new data sets
```{r}
# This data provides state-specific numbers regarding the number of drug
# overdose deaths per state as well as the predicted numbers for drug overdose
# deaths. I am super excited to plot this graph onto a map and look at how the
# worst affected states change over time. I am also interested in seeing whether
# trends match across drug groups.

drugoverdosedeaths<- read.csv("VSRR_Provisional_Drug_Overdose_Death_Counts.csv") %>%
  clean_names() %>%
  select("state", "year", "month", "indicator", "data_value", "state_name", "predicted_value")
```
...
Future: I think I am content with the state specific data with drugs of choice but I think it would be cool to also map out opioid deaths based on country region or based on income levels in a location. I still have to flesh out this idea though.
